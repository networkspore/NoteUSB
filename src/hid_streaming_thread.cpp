...
void HIDStreamingThread::set_session(NoteUSBSession* session) {
    session_ = session;
    syslog(LOG_DEBUG, "HIDStreamingThread: session set for device %s", device_id_.c_str());
}
...
bool HIDStreamingThread::is_encryption_enabled() const {
    return session_ && session_->is_encryption_enabled();
}
...
void HIDStreamingThread::process_hid_report(const uint8_t* data, int length) {
    ...
    // Serialize and send (with encryption if enabled)
    std::vector<uint8_t> packet = event.serialize_with_header();
    
    // Encrypt if session is available and encryption is enabled
    if (session_ && is_encryption_enabled()) {
        std::vector<uint8_t> encrypted;
        if (session_->encrypt_data(packet, encrypted)) {
            packet = encrypted;
        } else {
            syslog(LOG_ERR, "HIDStreamingThread: encryption failed for device %s",
                   device_id_.c_str());
        }
    }
